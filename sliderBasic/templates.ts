...
<div class="slider-bar-control slider-lock-control">
    <md-button
        ...
        aria-label="{{ !ctrl.isLocked ? 'plugins.sliderBasic.bar.lock' : 'plugins.sliderBasic.bar.unlock' | translate }}"
        class="md-icon-button primary"
        ng-click="ctrl.lock()">
        <md-tooltip>{{ !ctrl.value.isLocked ? 'plugins.sliderBasic.bar.lock' : 'plugins.sliderBasic.bar.unlock' | translate }}</md-tooltip>
        <md-icon ng-if="ctrl.value.isLocked">${createSVG('lock')}</md-icon>
        <md-icon ng-if="!ctrl.value.isLocked">${createSVG('lockOpen')}</md-icon>
    </md-button>
</div>`;

export const LOOP_BAR_TEMPLATE = `
<div class="slider-bar-control slider-loop-control">
    <md-button
        ng-controller="LoopSliderCtrl as ctrl"
        aria-label="{{ !ctrl.isLooped ? 'plugins.sliderBasic.bar.loop' : 'plugins.sliderBasic.bar.unloop' | translate }}"
        class="md-icon-button primary"
        ng-click="ctrl.loop()">
        <md-tooltip>{{ !ctrl.value.isLooped ? 'plugins.sliderBasic.bar.loop' : 'plugins.sliderBasic.bar.unloop' | translate }}</md-tooltip>
...
<div class="slider-bar-control slider-play-control">
    <md-button
        ...
        aria-label="{{ 'plugins.sliderBasic.bar.previous' | translate }}"
        class="md-icon-button primary"
        ng-click="ctrl.step('down')">
        <md-tooltip>{{ 'plugins.sliderBasic.bar.previous' | translate }}</md-tooltip>
        <md-icon>${createSVG('previous')}</md-icon>
    </md-button>
    <md-button
        ng-controller="PlaySliderCtrl as ctrl"
        aria-label="{{ ctrl.value.isPlaying ? 'plugins.sliderBasic.bar.pause' : 'plugins.sliderBasic.bar.play' | translate }}"
        class="md-icon-button primary"
        ng-click="ctrl.play()">
        <md-tooltip>{{ ctrl.value.isPlaying ? 'plugins.sliderBasic.bar.pause' : 'plugins.sliderBasic.bar.play' | translate }}</md-tooltip>
        <md-icon ng-if="ctrl.value.isPlaying">${createSVG('pause')}</md-icon>
        <md-icon ng-if="!ctrl.value.isPlaying">${createSVG('play')}</md-icon>
    </md-button>
    <md-button
        ng-controller="StepSliderCtrl as ctrl"
        aria-label="{{ 'plugins.sliderBasic.bar.foward' | translate }}"
        class="md-icon-button primary"
        ng-click="ctrl.step('up')">
        <md-tooltip>{{ 'plugins.sliderBasic.bar.foward' | translate }}</md-tooltip>
...
<div class="slider-bar-control slider-refresh-control">
    <md-button
        ...
        aria-label="{{ 'plugins.sliderBasic.bar.refresh' | translate }}"
        class="md-icon-button primary rv-slider-refresh"
        ng-click="ctrl.refresh()">
        <md-tooltip>{{ 'plugins.sliderBasic.bar.refresh' | translate }}</md-tooltip>
...
<div ng-controller="DelaySliderCtrl as ctrl" class="slider-bar-control slider-delay-control">
    <md-input-container class="md-block" md-no-float flex>
        <label>{{ 'plugins.sliderBasic.bar.delay' | translate }}</label>
        <md-select
            aria-label="{{ 'plugins.sliderBasic.bar.delay' | translate }}"
