...
var login = /** @class */ (function () {
    ...
    login.prototype.setidUTtheme = function (theme) {
        var json = "";
        var newUrl = url_1.urlgetidWu + theme;
        var output = this._conn.connexionAPI(this.gettoken(), json, newUrl, 'Get');
        //console.log(output.value);
        alert(theme);
        this._idUt._theme = theme;
        this._idUt._wUnit = output.value;
    ...
    login.prototype.setidUtToDDL = function (theme) {
        /*let listtest = {
        ...
        var list = [];
        var rank;
        for (var i in this._themeAcc) {
            if (this._themeAcc[i] === theme) {
                rank = i;
            }
        }
        for (var j in this._idUt[rank]._wUnit) {
