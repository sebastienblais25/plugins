...
var SliderBar = /** @class */ (function () {
    function SliderBar(mapApi, config) {
        ...
        nouislider.create(this._slider, {
            ...
            connect: true,
            behaviour: 'drag-tap',
            tooltips: [{ to: function (value) { return _this.formatPips(value, _this._config.type, _this._config.language); }, from: Number },
                { to: function (value) { return _this.formatPips(value, _this._config.type, _this._config.language); }, from: Number }],
            range: this.setRanges(mapWidth, config.limit, delta),
            pips: {
                mode: 'range',
                density: (mapWidth > 800) ? 2 : 25,
                format: {
                    to: function (value) { return _this.formatPips(value, _this._config.type, _this._config.language); },
                    from: Number
                }
            }
        });
    ...
    SliderBar.prototype.setDefinitionQuery = function (range) {
        for (var _i = 0, _a = this._config.layers; _i < _a.length; _i++) {
            var layer = _a[_i];
            var myLayer = this._mapApi.layers.getLayersById(layer.id)[0];
            var layerType = myLayer.type;
            if (layerType === 'esriDynamic' || layerType === 'esriFeature') {
                if (this._config.type === 'number') {
                    myLayer.setFilterSql('rangeSliderNumberFilter', layer.field + " > " + range.min + " AND " + layer.field + " <= " + range.max);
                }
                else if (this._config.type === 'date') {
                    var dates = this.getDate(range);
                    myLayer.setFilterSql('rangeSliderDateFilter', layer.field + " > DATE '" + dates[0] + "' AND " + layer.field + " <= DATE '" + dates[1] + "'");
                }
            }
                ...
                if (this._config.type === 'number') {
                    myLayer.esriLayer.setCustomParameters({}, { 'layerDefs': "{'" + myLayer._viewerLayer._defaultFC + "': '" + layer.field + " > " + range.min + " AND " + layer.field + " <= " + range.max + "'}" });
                }
                else if (this._config.type === 'date') {
                    var dates = this.getDate(range);
                    myLayer.esriLayer.setCustomParameters({}, { 'layerDefs': "{'" + myLayer._viewerLayer._defaultFC + "': \"" + layer.field + " > DATE '" + dates[0] + "' AND " + layer.field + " < DATE '" + dates[1] + "'\"}" });
                }
                else if (this._config.type === 'wmst') {
                    var dates = this.getDate(range, 'wmst');
                    myLayer.esriLayer.setCustomParameters({}, { 'TIME': dates[0] + "/" + dates[1] });
                }
            }
        }
    };
    SliderBar.prototype.getDate = function (range, type) {
        if (type === void 0) { type = 'esri'; }
        ...
        if (type === 'esri') {
            ...
            dateMin = min.getFullYear() + "-" + (min.getMonth() + 1).toString().padStart(2, '0') + "-" + min.getDate().toString().padStart(2, '0') + "T00:00:00";
            dateMax = max.getFullYear() + "-" + (max.getMonth() + 1).toString().padStart(2, '0') + "-" + max.getDate().toString().padStart(2, '0') + "T00:00:00";
        }
