...
const details_and_zoom_buttons_1 = require("./details-and-zoom-buttons");
const panel_rows_manager_1 = require("./panel-rows-manager");
const panel_status_manager_1 = require("./panel-status-manager");
const grid_accessibility_1 = require("./grid-accessibility");
...
class PanelManager {
    ...
    set panelStateManager(newPanelStateManager) {
    ...
    open(tableOptions, layer, tableBuilder) {
        if (this.currentTableLayer === layer) {
            ...
            this.tableOptions.onCellFocused = (cell) => {
                if (cell.rowIndex !== null) {
                    const focusedCell = $(`[row-index=${cell.rowIndex}]`).find(`[col-id=${cell.column.colId}]`)[0];
                    const focusedCellText = focusedCell.children[0];
            ...
            let mobileMenuTemplate = $(templates_1.MOBILE_MENU_TEMPLATE)[0];
            ...
            let recordCountTemplate = $(templates_1.RECORD_COUNT_TEMPLATE);
    ...
    updateColumnVisibility() {
        const columnStates = this.tableOptions.columnApi.getColumnState();
            ...
            this.noActiveFilters = function () {
                if (that.tableOptions.api !== undefined) {
                    ...
                    let noFilters = !columns.some(col => {
                        const columnConfigManager = new config_manager_1.ColumnConfigManager(that.configManager, col);
